...
public class TeamService : ITeamService
{
    ...
    public async Task UpdateAsync(int id, Teams team)
    {
        ...
        if (existingTeam.Name != team.Name)
        {
            ...
            {
                throw new InvalidOperationException( //Excepción que ndica que la operación no se puede ejecutar
                $"Ya existe un equipo con el nombre '{team.Name}'");
            }
        }
    ...
    public async Task DeleteAsync(int id)
    {
        ...
        if (!exists)
        {
            ...
            throw new KeyNotFoundException( //Excepción que indica que no se encontro la llave
            $"No se encontró el equipo con ID {id}");
        }
